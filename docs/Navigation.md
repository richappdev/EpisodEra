...
Unknown paths redirect to `/` (signed-in users then bounce to `/home`).
...
| Landing | `/`, `/landing` → `/` | `LandingPage` via `RootRoute` | Static copy + public trending posters |
| Trending / Home | `/home` | `DiscoveryPage` | `GET /trending/tv` or `GET /trending/movie`, `GET /discover/suggestions` |
...
- Marketing landing is the root at `/`; signed-in visitors are redirected to `/home`.
- Trending / Home lives at `/home` and remains publicly browsable when signed out.
